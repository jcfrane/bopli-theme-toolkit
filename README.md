...
Every theme supplies paired `.vue` and `.bopli.ts` templates only under `resources/js/templates/pages` and `resources/js/templates/entries`, with at least one ordinary Page and one generic Entry template and exactly one default of each kind. The companion imports typed helpers from `@bopli/theme-sdk/authoring`; the CLI compiles its small `field.*` declarations into protocol metadata and concrete generated props. `bopli-theme add page about` scaffolds a Page pair. Native Blog helpers follow the same paired-default rule. Legacy `blogs` and `posts` roots and inline `<bopli>` blocks are rejected. Entry projection fields cannot shadow Bopli metadata such as `url`, SEO, or adjacent-navigation keys. Page slot declarations are obsolete and rejected.
