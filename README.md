...
Themes may add a bounded `resources/bopli/starter.json` recipe. The CLI validates its Content Models, fields, routes, Entries, Pages, bindings, Blog setting, and template references before embedding it in the release descriptor. Bopli applies this recipe per Site during provisioning or an explicit Appearance import; globally installing a release never creates tenant content.
